...
- DotNetEnv (3.1.1): nạp biến môi trường từ file `.env` cho local development
...
- Microsoft.EntityFrameworkCore.Design (9.0.15): hỗ trợ EF Core tools ở startup project khi chạy migration/update database
...
Mục đích:
- Nạp biến môi trường từ `.env` trong môi trường local
- Xác thực JWT
- Tài liệu API với Swagger/OpenAPI
- Hỗ trợ EF Core tooling từ startup project API (PMC/CLI)
...
- EFCore.NamingConventions (9.0.0): tự động chuẩn hóa tên bảng/cột theo convention (ví dụ snake_case)
...
- Chuẩn hóa naming convention giữa model và database
- Kết nối PostgreSQL
